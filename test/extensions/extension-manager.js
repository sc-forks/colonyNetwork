...
contract("ExtensionManager", accounts => {
  ...
  before(async () => {
    ...
    await extensionManager.addExtension(COLONY_EXTENSION, 0, extensionResolver.address, [FUNDING_ROLE, ADMINISTRATION_ROLE]);
  });
  ...
  describe("without the extension manager", () => {
    it("allows a colony to install an extension manually (and without a resolver)", async () => {
      ...
      await checkErrorRevert(extension.install(colony.address, ethers.constants.AddressZero), "colony-extension-already-installed");
  ...
  describe("adding extensions", () => {
    it("allows the meta colony to add new extensions to the manager", async () => {
      ...
      await extensionManager.addExtension(extensionId, 0, resolver.address, []);
      await extensionManager.addExtension(extensionId, 1, resolver.address, []);
    });
    ...
    it("does not allow the meta colony to overwrite existing extensions", async () => {
      ...
      await checkErrorRevert(extensionManager.addExtension(extensionId, 0, resolver.address, []), "colony-extension-manager-extension-already-added");
    });
    ...
    it("does not allow the meta colony to add versions out of order", async () => {
      ...
      await checkErrorRevert(extensionManager.addExtension(extensionId, 1, resolver.address, []), "colony-extension-manager-bad-version");
    ...
    it("does not allow the meta colony to add a null resolver", async () => {
      ...
      await checkErrorRevert(
        extensionManager.addExtension(extensionId, 0, ethers.constants.AddressZero, []),
        "colony-extension-manager-bad-resolver"
      );
    });

    it.skip("does not allow other colonies to add new extensions to the manager", async () => {
      const fakeMetaColony = await IMetaColony(colony.address);
      const extensionId = soliditySha3(shortid.generate());
      const resolver = await Resolver.new();

      await checkErrorRevert(fakeMetaColony.addExtension(extensionId, 0, resolver.address, []), "colony-extension-manager-not-metacolony");
    });
  });
  ...
  describe("installing extensions (& user authorizations)", () => {
    ...
    it("does not allow an extension which requires root permission to be installed in a subdomain", async () => {
      ...
      const extensionId = soliditySha3(shortid.generate());
      const resolver = await Resolver.new();
      await extensionManager.addExtension(extensionId, 0, resolver.address, [ROOT_ROLE]);

      await checkErrorRevert(
        extensionManager.installExtension(extensionId, 0, colony.address, 0, 1, 0, 2, { from: ROOT }),
        "colony-extension-manager-bad-domain-for-root"
      );
    ...
    it("allows an extension to be installed with all roles", async () => {
      ...
      const extensionId = soliditySha3(shortid.generate());
      const resolver = await Resolver.new();
      await extensionManager.addExtension(extensionId, 0, resolver.address, allRoles);
      await extensionManager.installExtension(extensionId, 0, colony.address, 0, 1, 0, 1, { from: ROOT });
    });
    ...
    it("allows an extension to be installed with non-existent roles, without erroring", async () => {
      ...
      const extensionId = soliditySha3(shortid.generate());
      const resolver = await Resolver.new();
      await extensionManager.addExtension(extensionId, 0, resolver.address, [ROOT_ROLE, fakeRole]);
      await extensionManager.installExtension(extensionId, 0, colony.address, 0, 1, 0, 1, { from: ROOT });
